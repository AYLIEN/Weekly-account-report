...
def main(search_period_type_input="delta"):
    ...
    report = report.rename(columns={'account_id' : 'Account ID',
                                    ...
                                    'analytics_start_datetime' : 'Usage Data collected from (Timedate)',
                                    'analytics_end_datetime' : 'Usage Data collected to (Timedate)',
                                    'domain' : 'Account email domain',
                                    'average_hits_pe_day' : 'Total Hits per day (Average)', #Old name: 'Average hits per day',
    ...
    report['Usage Data collected to (Timedate)'] = report['Usage Data collected to (Timedate)'].apply(lambda x : x.split("T")[0] if(x is not None) else None)
    ...
    if save_to_excel is True:
        ...
        file_name = "accounts_report_" + str(datetime.now().strftime('%Y%m%d_%H%M%S')) + ".xlsx" 
